...
export function SettlementsPanel() {
  ...
  async function open(input: { anchor: string; asset: string; amount: number }) {
    setPending(true);
    await run(() => openSettlement(input), `Opened a settlement for ${input.amount} ${input.asset}.`);
    setPending(false);
  }
  ...
  const visibleSettlements =
    state.status === \"ready\"
      ? state.settlements.filter((s) => matchesQuery([s.id, s.anchor, s.asset], query))
      : [];

  return (
    <div className=\"space-y-6\">
      <div aria-live=\"polite\" className=\"sr-only\">{loadMoreAnnouncement}</div>
      <Card>
        <h2 className=\"mb-3 text-sm font-semibold text-zinc-200\">Open settlement</h2>
          ...
          <>
            ...
            {visibleSettlements.length === 0 && state.settlements.length > 0 ? (
              ...
              <SettlementTable
                ...
                onExecute={(id) => runSettlementAction(() => executeSettlement(id), `Executed settlement #${id}.`)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          if (id !== null) {
            runSettlementAction(() => cancelSettlement(id), `Cancelled settlement #${id}.`);
          }
        }}
      />
    </div>
